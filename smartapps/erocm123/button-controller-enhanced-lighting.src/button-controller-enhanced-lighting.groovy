	...
	dynamicPage(name: "selectButton", title: "First, select your button device", nextPage: null, uninstall: configured(), install: true) {
        ...
        section("Menu") {
                ...
                for (int i = 1; i <= (numberOfButtons as Integer); i++){
                   ...
                   if (settings["mode_${i}"] != null) { 
                      settings["mode_${i}"].each {
                         configDescription += " mode: ${ settings["mode_${i}"]}"
                      }
                   }                  
...
def getButtonSections(buttonNumber) {
	return {
        ...
        if (lightsConfigured != null) {
		   ...
		   section("Mode") {
            input "mode_${buttonNumber}", "mode", title: "change to this mode", multiple: false, required: false
...
def executeHandlers(buttonNumber) {
    ...
    if ((settings["mode_${buttonNumber}"])) {
              setLocationMode(settings["mode_${buttonNumber}"])
              	log.trace "change mode"
    }
